 ...
 * etc.
 *
 * Now supports usage with Custom_Picture_Controls.js!
 *
 * -------------------------------
 * Plugin Commands:
 * 
 ...
 *
 *      DisableBattleFades
 *      - Completely disables both the fade-out and fade-in during the
 *        transition of a battle.
 *
 *      EnableBattleFades
 *      - Re-enables both the fade-out and fade-in during the transition
 *        of a battle.
 *
 * -------------------------------
 * Script Calls:
...
var parameters = PluginManager.parameters('Geo_RemoveBattleFadeOut');
...
Scene_Battle.prototype.start = function() {
	if ($gameVariables._disableBattleFadeIn && beesList.battleFade.idList) {
		...
		for (let i = 0; i < picsList.length; i++) {
			...
			if (pic._frameData) {
				beesList.battleFade.setupPictureCustomFrames(pic._index, pic.width, pic.height, pic.hFrames, pic.vFrames);
				beesList.battleFade.setPictureFrameIndex(pic._index, pic._frameData.frameIndex);
			};
        }
		// Tint Manager;
...
if (_TDS_.CustomPictureControls) {
  beesList.battleFade.oldSetupPictureCustomFrames = Game_Interpreter.prototype.setupPictureCustomFrames;
  Game_Interpreter.prototype.setupPictureCustomFrames = function(pictureId, width, height, hFrames, vFrames) {
	$gameScreen.picture(pictureId).hFrames = hFrames;
	$gameScreen.picture(pictureId).vFrames = vFrames;
	$gameScreen.picture(pictureId).width = width;
	$gameScreen.picture(pictureId).height = height;
	beesList.battleFade.oldSetupPictureCustomFrames.call(this, pictureId, width, height, hFrames, vFrames);
  };
  
  beesList.battleFade.setupPictureCustomFrames = function(pictureId, width, height, hFrames, vFrames) {
  ...
  beesList.battleFade.setPictureFrameIndex = function(pictureId, index) {
    // Get Picture
    var picture = $gameScreen.picture(pictureId)
    // Set frame Index
    picture.setCustomFrameIndex(index)
    // Update Custom Frame
    SceneManager._scene._spriteset._pictureContainer.children[pictureId].updateCustomFrame();
  };
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
  ...
  case 'ImprovedBattleIntro':
	  ...
	  break;
  case 'DisableFadeOut':
    // Disable Battle FadeOut
    BattleManager._showEncounterEffect = undefined;
    break;
  case 'EnableFadeOut':
    // Enable Battle FadeOut
    BattleManager._showEncounterEffect = true;
    break;
  case 'DisableBattleFades':
    // Disable All Battle Fades
    BattleManager._showEncounterEffect = undefined;
	$gameVariables._disableBattleFadeIn = true;
    break;
  case 'EnableBattleFades':
    // Enable All Battle Fades
    BattleManager._showEncounterEffect = true;
	$gameVariables._disableBattleFadeIn = undefined;
    break;
  };
