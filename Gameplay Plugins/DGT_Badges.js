 ...
 * @help
 *
 * Renaming this file is recommended, so that multiple modded badge 
 * scripts may be loaded at once
 *
 * Plugin Commands:
 * 
 * for all purposes, modid is the part of the name of your yaml file after 
 * 'badgedata_' e.g. if you have a badge file called 'badgedata_coolmod', 
 * the mod id you would use in commands would be 'coolmod'
 * 
 ...
 * same functionality as plugin commands above, but will return true if 
 * "sucessful"; if the user already has the badge and you call unlockbadge, 
 * it will return false;  likewise, if the user does not have a badge and 
 * you call lockbadge, it returns false
 * DGT.isBadgeUnlocked("modId", "badgeName")
 * returns true if the badge is unlocked
 * DGT.totalUnlockedBadges('modId')
 * returns the total number of unlocked badges, if modId is specified, 
 * returns the number for that mod only
 * DGT.totalUnlockedBadges('modId')
 * returns the total number of locked AND unlocked badges, if modId is 
 * specified, returns the number for that mod only
 * DGT.badgeUnlockRatio('modId')
 * returns a number between 0 and 1 representing the portion of currently 
 * unlocked badges, if modId is specified, returns the number for that mod only
 ...
 *
 * TO USE:
 * copy the js file and the yaml file
 * rename the js file (it doesnt really matter what you rename it to, just as long as it doesnt match anything else)
 * (you can also use the randomize file name option of one loader)
 *
 * rename the yaml file to 'badgedata_modid.yaml', where modid is the id of your mod
 * edit the yaml to add your badges (more info is in the file for formatting)
 *
 *
 *
